        ...
        class AppStore {
            ...
            renderPlan() {
                ...
                this.state.goals.forEach(goal => {
                    ...
                    for (let w = 1; w <= 12; w++) {
                        // Filter tactics for this week
                        // Legacy tactics without week defaults to Week 1 or we migrate them? 
                        // Let's assume default to currentWeek if undefined, or Week 1. 
                        // For safe migration: if(!t.week) t.week = 1;
                        const tactics = goal.tactics.filter(t => (t.week || 1) == w);
                        const isCurrent = w === this.state.currentWeek;

                        html += `
                            <div style="border:1px solid #eee; border-radius:8px; overflow:hidden;">
                                <div class="flex-between" style="padding:10px 16px; background:${isCurrent ? '#fff0f0' : '#fafafa'}; cursor:pointer;" onclick="app.toggleWeekAccordion('${goal.id}', ${w})">
                                    <span style="font-weight:600; font-size:0.9rem; color:${isCurrent ? 'var(--primary-red)' : '#666'};">Week ${w}</span>
                                    <div style="display:flex; align-items:center; gap:8px;">
                                        <span class="text-sub text-sm">${tactics.length} tactics</span>
                                        <i data-lucide="chevron-down" size="16"></i>
                                    </div>
                                </div>
                                <div id="accordion-${goal.id}-w${w}" style="display:${isCurrent ? 'block' : 'none'}; padding:16px; background:#fff; border-top:1px solid #eee;">
                                    ${tactics.map(t => `
                                        <div class="flex-between" style="padding:4px 0; border-bottom:1px solid rgba(0,0,0,0.05);">
                                    ...
                                    `).join('')}
                                    <button class="text-sub text-sm" style="margin-top:8px; color:var(--primary-red);" onclick="app.addTactic('${goal.id}', ${w}, false)">+ Add Tactic to Week ${w}</button>
                                </div>
                            </div>
                        `;
                    }
                ...
                lucide.createIcons(); // Ensure icons are rendered after new HTML is added
            }

            toggleWeekAccordion(goalId, week) {
                const el = document.getElementById(`accordion-${goalId}-w${week}`);
                if (el) el.style.display = el.style.display === 'none' ? 'block' : 'none';
                lucide.createIcons(); // Re-render icons in case new ones appear or old ones need refreshing
            }
            ...
            addTactic(goalId, week, isRecurring) {
                const goal = this.state.goals.find(g => g.id === goalId);
                if (goal) {
                    const title = prompt(isRecurring ? 'Enter Recurring Tactic (will create for Weeks 1-12):' : `Enter Tactic for Week ${week}:`);
                    if (title) {
                        if (isRecurring) {
                        ...
                        this.save();
                        this.render(); // This will default reload tabs to current state

                        // If we are in Plan tab, we want to expand the specific week? 
                        // The toggleWeekAccordion logic preserves state if we don't clear innerHTML... 
                        // But renderPlan clears innerHTML. 
                        // A nice to have: auto-open the week we just added to. 
                        // Implementing auto-open via setTimeout?
                        if (week) setTimeout(() => this.toggleWeekAccordion(goalId, week), 100);
                    }
                }
            }
